...
var decodeHexAddress = function (hexAddress) {
    ...
    var addressBytes = Buffer.from(hexAddress, 'hex');
    var words = bech32.toWords(addressBytes);
    var prefix;
                                                ...
                                                setValuesAsync = function () { return __awaiter(_this, void 0, void 0, function () {
                                                    ...
                                                    return __generator(this, function (_a) {
                                                        switch (_a.label) {
                                                            ...
                                                            case 5:
                                                                cborBalance = _a.sent();
                                                                balance = decode(Buffer.from(cborBalance, 'hex'));
...
function useCardano(props) {
    var _this = this;
    var dAppConnect = useRef(null);
    var _a = useState(Wallet.meerkatAddressObserver.get()), meerkatAddress = _a[0], setMeerkatAddress = _a[1];
    var _b = useState(Wallet.enabledObserver.get()), isEnabled = _b[0], setIsEnabled = _b[1];
    var _c = useState(Wallet.isConnectingObserver.get()), isConnecting = _c[0], setIsConnecting = _c[1];
    var _d = useState(Wallet.enabledWalletObserver.get()), enabledWallet = _d[0], setEnabledWallet = _d[1];
    var _e = useState(Wallet.stakeAddressObserver.get()), stakeAddress = _e[0], setStakeAddress = _e[1];
    var _f = useState(Wallet.usedAddressesObserver.get()), usedAddresses = _f[0], setUsedAddresses = _f[1];
    var _g = useState(Wallet.unusedAddressesObserver.get()), unusedAddresses = _g[0], setUnusedAddresses = _g[1];
    var _h = useState(Wallet.accountBalanceObserver.get()), accountBalance = _h[0], setAccountBalance = _h[1];
    var _j = useState(Wallet.installedWalletExtensionsObserver.get()), installedExtensions = _j[0], setInstalledExtensions = _j[1];
    var _k = useState(Wallet.isConnected.get()), isConnected = _k[0], setIsConnected = _k[1];
    var _l = useState(Wallet.lastConnectedWallet.get()), lastConnectedWallet = _l[0], setLastConnectedWallet = _l[1];
    ...
    var initDappConnect = useCallback(function (dAppName, dAppUrl, verifyConnection, onApiInject, onApiEject) {
    ...
    var connectToWallet = useCallback(function (walletName, retries, retryIntervalInMs) {
        if (retries === void 0) { retries = 20; }
        if (retryIntervalInMs === void 0) { retryIntervalInMs = 25; }
    ...
    var checkEnabled = useCallback(function () { return __awaiter(_this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            Wallet.checkEnabled(limitNetwork);
            return [2 /*return*/];
        });
    }); }, [lastConnectedWallet]);
    var signMessage = useCallback(function (message, onSignMessage, onSignError) { return __awaiter(_this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            Wallet.signMessage(message, onSignMessage, onSignError);
            return [2 /*return*/];
        });
    }); }, [isEnabled, enabledWallet, limitNetwork]);
    var connect = useCallback(function (walletName, onConnect, onError) { return __awaiter(_this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            if (isConnecting)
                return [2 /*return*/];
...
var ModalDialog = function (props) {
    ...
    return (jsx(Modal, __assign({ onClick: function () {
            if (typeof handleClose === 'function') {
                handleClose();
            }
        }, style: { display: visible ? 'block' : 'none' } }, { children: jsxs(ModalContent, __assign({ "data-testid": "modal-content" }, { children: [jsx(QRCode, { removeQrCodeBehindLogo: true, logoImage: icon ? icon : undefined, ecLevel: "H", eyeRadius: 5, qrStyle: "dots", value: content }), jsx("p", __assign({ style: { maxWidth: 300 } }, { children: "You can use a CIP45 compliant mobile wallet on your smartphone (e.g. Eternl) to connect an external peer to peer wallet" }))] })) })));
};
...
var ConnectWalletButton = function (_a) {
    ...
    var _p = useState(false), showModalDialog = _p[0], setShowModalDialog = _p[1];
    useEffect(function () {
    ...
    var walletMenu = (jsxs(Menu$1, __assign({ id: "connect-wallet-menu" }, { children: [peerConnectEnabled && (jsxs(MenuItem$1, __assign({ borderRadius: borderRadius, primaryColor: themeColorObject.hex(), primaryColorLight: themeColorObject.mix(Color('white'), 0.9).hex(), onClick: function () { return setShowModalDialog(true); } }, { children: [jsx(MenuItemIcon$1, { src: getWalletIcon('peer-connect') }), "Link Wallet"] }))), availableWallets ? (availableWallets.map(function (availableWallet) {
                if (isMobile &&
                    !mobileWallets.includes(availableWallet.toLowerCase())) {
                    return (jsxs(DesktopMenuItem$1, __assign({ borderRadius: borderRadius }, { children: [jsx(MenuItemIcon$1, { src: getWalletIcon(availableWallet) }), capitalize(availableWallet), jsx("span", { children: "Desktop Only" })] }), availableWallet));
                }
                return (jsxs(MenuItem$1, __assign({ borderRadius: borderRadius, primaryColor: themeColorObject.hex(), primaryColorLight: themeColorObject
                        .mix(Color('white'), 0.9)
                        .hex(), onClick: function () { return connectMobileWallet(availableWallet); } }, { children: [jsx(MenuItemIcon$1, { src: getWalletIcon(availableWallet) }), capitalize(availableWallet)] }), availableWallet));
            })) : (jsx("span", __assign({ id: "connect-wallet-hint" }, { children: "Please install a wallet browser extension (".concat(formatSupportedWallets(supportedWallets), " are supported)") })))] })));
    var actionMenu = hideActionMenu ? null : (jsxs(Menu$1, __assign({ id: "connect-wallet-menu" }, { children: [typeof message === 'string' && (jsx(MenuItem$1, __assign({ borderRadius: borderRadius, primaryColor: themeColorObject.hex(), primaryColorLight: themeColorObject.mix(Color('white'), 0.9).hex(), onClick: function () { return signMessage(message, onSignMessage); } }, { children: "Sign a message" }))), customActions.map(function (customAction, index) { return (jsx(MenuItem$1, __assign({ borderRadius: borderRadius, primaryColor: themeColorObject.hex(), primaryColorLight: themeColorObject.mix(Color('white'), 0.9).hex(), onClick: customAction.onClick }, { children: customAction.label }), "custom-action-".concat(index))); }), jsx(MenuItem$1, __assign({ borderRadius: borderRadius, primaryColor: themeColorObject.hex(), primaryColorLight: themeColorObject.mix(Color('white'), 0.9).hex(), onClick: function () {
                    disconnect();
...
var ConnectWalletList = function (_a) {
    ...
    var themeColorObject = primaryColor
        ? Color(primaryColor)
        : Color('#0538AF');
    return (jsx(Menu, __assign({ customCSS: customCSS, "data-testid": "connect-wallet-list" }, { children: availableWallets ? (availableWallets.map(function (availableWallet) {
            if (isMobile &&
                !mobileWallets.includes(availableWallet.toLowerCase())) {
                return (jsxs(DesktopMenuItem, __assign({ primaryColor: themeColorObject.hex(), primaryColorLight: themeColorObject
                        .mix(Color('white'), 0.9)
                        .hex(), borderRadius: borderRadius, gap: gap }, { children: [jsx(MenuItemIcon, { src: getWalletIcon(availableWallet) }), capitalize(availableWallet), jsx("span", { children: "Desktop Only" })] }), availableWallet));
            }
            return (jsxs(MenuItem, __assign({ primaryColor: themeColorObject.hex(), primaryColorLight: themeColorObject
                    .mix(Color('white'), 0.9)
                    .hex(), borderRadius: borderRadius, gap: gap, onClick: function () { return connectMobileWallet(availableWallet); } }, { children: [jsx(MenuItemIcon, { src: getWalletIcon(availableWallet) }), capitalize(availableWallet)] }), availableWallet));
        })) : (jsx("span", { children: "Please install a wallet browser extension (".concat(formatSupportedWallets(supportedWallets), " are supported)") })) })));
};

export { ConnectWalletButton, ConnectWalletList, useCardano };
